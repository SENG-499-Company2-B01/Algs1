...
def cat_seek(x: list,classes_no: int,teachers_no: int,TEPW: float,ITDW: float,ICDW: float):
    ...
    #the above code are just initialization
	for cp in range(SMP):
		copy_matrices(0, 35, temp_cat, cat_copy[cp], classes_no)
		if ((consider == 0) or ((consider == 1) and (cp != SMP - 1))):
            # generating 2 list of random integers
			unique_randint(hd, 0, 26-1, timeslots_to_change)
            # there is a problem for these integers that it may encounter unscheduled blocks e.g. global_best[hd2[aa]][hd[aa]]=0
            # this may cause some of the insert_column() operation not making changes            
            # the previous function did not handle this issue, so I believe it is designed to be so
            ...
            # I don't understand the purpose of this for loop, it seems that the porgram is just swaping teaching schedules randomly
            # and hoping to find a better cat
            ...
            # the last fitness spot designed for self position consideration
	...
	if (fsmax == fsmin):
		all_equal = 1
    # the final switch, result is returned to list x
    # it is the local x, not the global x
	if (all_equal == 1):
